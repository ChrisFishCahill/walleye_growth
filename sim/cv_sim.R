#-----------------------------
#Simulation testing 10% lakes-fold cross validation scoring metrics
#Considered predictive negative log likelihood, RMSE, and RMSLE
#Coded by Cahill May 2019
#-----------------------------
...
K = 10
lakes_to_predict = sample(unique(N_lakes), replace=F)
by <- round((K/100)*N_lakes)

to <- seq(from=1, to=N_lakes-by, by=by)

spatial <- nonspatial <- matrix(NA, Nsim, ncol=length(to))
cv_spatial <- cv_nonspatial <- rmsle_spatial <- rmsle_nonspatial <-
  rmse_spatial <- rmse_nonspatial <- matrix(NA, Nsim, ncol=length(to))
...
while(replicate < (Nsim+1) ){
        ...
        mesh = inla.mesh.2d(loc=loc_xy, max.edge = c(0.5,1)*2 )

        spde = inla.spde2.matern( mesh, alpha=2 )

        lakes_to_predict = sample(unique(DF$Lake), replace=F)
        ...
        for(k in to){
          # lakes for first K-1 partitions
          if(k !=max(to)) Partition_i = ifelse(DF$Lake %in% lakes_to_predict[k:(k + by - 1)],1,0)
          # lakes in partition == K
          if(k == max(to)) Partition_i = ifelse(DF$Lake %in% lakes_to_predict[k:(k + by)],1,0)
        ...
        data_spatial = list("Nobs"=nrow(DF), "length_i"=DF$Simulated_Length, "age_i" = DF$Age,
                            ...
                            "Nlakes" =  length(unique(DF$Lake)),
                            "spdeMatrices" = spdeMatrices, "s_i" = DF$Lake-1, "t_i" = DF$Year-1,
                            "CTL" = CTL, "predTF_i"=Partition_i)
        ...
        data_nonspatial = list("Nobs"=nrow(DF), "length_i"=DF$Simulated_Length, "age_i" = DF$Age,
                               ...
                               "Nlakes" =  length(unique(DF$Lake)),"CTL" = CTL, "predTF_i"=Partition_i )
        ...
        if(!inherits(opt_spatial, "error") && !inherits(opt_nonspatial, "error") &&
          ...
          spatial[replicate, which(to==k)] <- spatial_report$pred_jnll / sum (Partition_i)
          rmsle_spatial[replicate, which(to==k)] <- spatial_report$RMSLE
          rmse_spatial[replicate, which(to==k)] <- spatial_report$RMSE
          cv_spatial[replicate, which(to==k)] <- exp(opt_spatial$par["ln_cv"])

          nonspatial[replicate, which(to==k)] <- nonspatial_report$pred_jnll / sum (Partition_i)
          rmsle_nonspatial[replicate, which(to==k)] <- nonspatial_report$RMSLE
          rmse_nonspatial[replicate, which(to==k)] <- nonspatial_report$RMSE
          cv_nonspatial[replicate, which(to==k)] <- exp(opt_nonspatial$par["ln_cv"])
     } #try
  }
...
#double check the rmse / rmsle calculations:
predicted <- spatial_report$length_pred[which(Partition_i==1)]
observed <- DF$Simulated_Length[which(Partition_i==1)]
...
rmsle <- function(observed, predicted){
  se <- (log(observed + 1)-log(predicted + 1))^2
  sqrt(mean(se))
}
...
spatial_report$RMSLE

png( file="cv_sim_results.png", width=6, height=6, res=200, units="in")
par(mfrow=c(2,2), mar=c(4,4,2,2))
hist(cv_spatial, main="", xlab="Coefficient of Variation")

hist(rowMeans(spatial)-rowMeans(nonspatial), main="", xlab="Difference in Pred NLL")

hist(rowMeans(rmse_spatial)-rowMeans(rmse_nonspatial), main="",
     xlab="Difference in RMSE")

hist(rowMeans(rmsle_spatial)-rowMeans(rmsle_nonspatial), main="",
     xlab="Difference in RMSLE")
dev.off()
